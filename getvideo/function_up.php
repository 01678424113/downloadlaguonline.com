...
function luuanh($image, $name, $casi, $type, $width, $height) {
    ...
    if ($image != '') {
        ...
        if (preg_match('|image|', $data3)) {
            ...
            if (!file_exists("../icon_video")) {
                mkdir("../icon_video");
            }
            if (!file_exists("../icon_video/$nam")) {
                mkdir("../icon_video/$nam");
            }
            if (!file_exists("../icon_video/$nam/$thang")) {
                mkdir("../icon_video/$nam/$thang");
            }
            if (!file_exists("../icon_video/$nam/$thang/$ngay")) {
                mkdir("../icon_video/$nam/$thang/$ngay");
            }
            ...
            $filename = "../icon_video/$nam/$thang/$ngay/$name.png";
...
function luuanh_content($image, $name, $type) {
    ...
    if ($image != '') {
        ...
        if (preg_match('|image|', $data3)) {
            ...
            if (!file_exists("../icon_video_content")) {
                mkdir("../icon_video_content");
            }
            if (!file_exists("../icon_video_content/$nam")) {
                mkdir("../icon_video_content/$nam");
            }
            if (!file_exists("../icon_video_content/$nam/$thang")) {
                mkdir("../icon_video_content/$nam/$thang");
            }
            if (!file_exists("../icon_video_content/$nam/$thang/$ngay")) {
                mkdir("../icon_video_content/$nam/$thang/$ngay");
            }
            ...
            $filename = "../icon_video_content/$nam/$thang/$ngay/$name.png";
            $content = file_get_contents($image);
            $savefile = fopen($filename, 'w');
            $chuan = fwrite($savefile, $content);
            fclose($savefile);
...
function them_link_content($name, $casi, $duration, $cat, $type, $conn, $domain, $name_domain) {
    ...
    $row = mysqli_fetch_array(mysqli_query($conn, "SELECT id,url,title,name FROM `baihat` WHERE id=$offset LIMIT 1"));
    if ($row['name'] == $name) {
        return '';
    }
    if (empty($row)) {
