...
$fcache_side = 'sidebar';
include_once 'function/fcache_sidebar.php';
?>
            ...
            <div id="song-list_ID">
				...
				for ($i = 0; $i < count($json_data); $i++) {
                    ...
                    if (empty($row['id'])) {
						...
						$link = str_replace('https://lagu123.blog/', '', $urlgoc);
...
<div class="sidebar-box sidebar-video">
    ...
    <div class="box-bxh sidebar-charts">
        <div class="tab-content active" id="indo-vi">
            <?php
			$name_file_cache = 'sidebar_video';
			$check_data = cache_check_side($name_file_cache,$fcache_side);
			if($check_data == 0) {
				$data_side_video = [];
				$query_side_video = mysqli_query($conn, "SELECT id,url,icon,alt_anh,name,author FROM video WHERE theloai LIKE '%Indo%' ORDER BY views DESC LIMIT 10");
				foreach ($query_side_video as $row) {
					$json = array(
						'id' => $row['id'],
						'name' => $row['name'],
						'author' => $row['author'],
					...
					<?php
					array_push($data_side_video, $json);
				}
				cache_save_side($name_file_cache,$fcache_side,$data_side_video);
			} else {
				$row = $check_data;
				for ($i = 0; $i < count($row); $i++) {
					...
					<div class="song-list">
						...
						<p class="singer"><?php echo $row[$i]['author']; ?></p>
					</div>
					<?php
				}
			}
