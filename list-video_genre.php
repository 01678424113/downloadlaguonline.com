...
if (empty($_GET['p'])) {
	$p = 1;
	$geturl = $home . '/video/' . $type_p;
} else {
	$p = $_GET['p'];
	$geturl = $home . '/video/' . $type_p . '/' . $p;
}
$get_country = $home . '/video/' . $type_p;
                        ...
                        <div class="content-music">
                            ...
                            <div class="content-box">
                                <div class="home-title page_title">
                                    <h2 class="title">Video <?php echo $type_p; ?></h2>
                                </div>
                                <div class="playlist rows">
                                    <div class="row">
                                        <?php
                                        $offset_result = mysqli_query($conn, "SELECT COUNT(id) AS `offset` FROM `video` WHERE cat LIKE '%" . $type . "%'");
                                        $offset_row = mysqli_fetch_object($offset_result);
                                        $total = $offset_row->offset;
                                        if ($p > 0) {
                                            $show = 52;
                                            if ($total <= $show) {
                                                $query = mysqli_query($conn, "SELECT id,name,url,icon,alt_anh,views,duration,author FROM `video` WHERE cat LIKE '%" . $type . "%' ORDER BY views DESC limit $total");
                                                ...
                                                if ($p < 1)
                                                    $p = 1;
                                                $j = ($p - 1) * $show;
                                                $query = mysqli_query($conn, "SELECT id,name,url,icon,alt_anh,views,duration,author FROM `video` WHERE cat LIKE '%" . $type . "%' ORDER BY views DESC limit $j,$show");
                                                while ($row = mysqli_fetch_array($query)) {
                                                    ?>
                                                    <div class="col-md-3 col-sm-3 col-xs-6 list">
                                                        <div class="home-img">
