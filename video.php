...
mysqli_query($conn, "UPDATE `video` SET `views`=`views`+1 WHERE id=$id");

$fcache = 'video';
include 'function/fcache.php';
$name_file_cache = 'video';
$check_data = cache_check($id,$name_file_cache,$fcache);
if($check_data == 0) {
	$row = mysqli_fetch_array(mysqli_query($conn, "SELECT api,name,author,title,description,url,content,image,icon,alt_anh,views FROM video WHERE id=$id LIMIT 1"));
...
if ($row['url'] != $url) {
    $direct = $home . '/video/' . $id . '/' . $row['url'] . '.html';
...
$geturl = $home . '/video/' . $id . '/' . $row['url'] . '.html';
$anh_share = $row['image'];
include 'inc/header.php';
include_once 'function/func_se.php';
pk_stt2_function_wp_head_hook($id, $conn, 'lastsearch_video', 'video', $url);

$index = unserialize(file_get_contents("up/index.txt"));
$title_index = $index['title'];
$list_bh = unserialize(file_get_contents("up/list-video.txt"));
                ...
                <div class="col-content content animated">
                    <div class="col-md-8 col-sm-8 col-xs-12 content-col-left">
                        <div class="single-music">
                            <div class="video-box">
                                <div class="video">
                                    <iframe class="v_frame" src="https://www.youtube.com/embed/<?php echo $row['api']; ?>?rel=0&autoplay=1" frameborder="0" width="100%" allowfullscreen></iframe>
                                </div>
                            </div>
                            ...
                            <div class="box-menu-player">
                                <ul>
                                    <li><a target="_blank" href="<?php echo $home . '/download-video/' . $row['api']; ?>" id="btnDownloadPlaylist"><span class="icon-download"></span>Download</a></li>
                            ...
                            <div class="col-md-12 col-sm-12 col-xs-12 lyrics">
                                <h2 class="h2_content">Download video lagu <?php echo $name; ?> Mp3 Terbaru Gratis</h2>
                            ...
                            <div class="tags-title-link">
                                ...
                                <?php
                                $sqlse = mysqli_query($conn, "SELECT tukhoa FROM `lastsearch_video` WHERE `post_id` = '$id' ORDER BY `count` DESC LIMIT 10");
                                $cari = '';
