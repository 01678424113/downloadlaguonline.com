...
include 'function/function_up.php';
include 'function/function_video.php';
include 'function/image.php';

$api = $_GET['api'];
$row = mysqli_fetch_array(mysqli_query($conn, "SELECT id,url FROM video WHERE api='$api' LIMIT 1"));
if (!empty($row['id'])) {
    header("Location:" . $home . '/video/' . $row['id'] . '/' . $row['url'] . '.html');
    exit();
}
...
$url = url($name, '', 'video');
$alt_anh = title($name, $author, 'video');
...
$quer = mysqli_fetch_array(mysqli_query($conn, "SELECT id,url FROM `video` WHERE api='" . $api . "' ORDER BY id DESC LIMIT 1"));
if (empty($quer)) {
    header("Location:" . $home);
    exit();
}
header("Location:" . $home . "/video/" . $quer['id'] . "/" . $url . ".html");
exit();
?>